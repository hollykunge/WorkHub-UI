  ...
  <div class="activity-list">
    <div class="activity-list-container">
      <div class="message-container">
        <!-- 使用统一的数据展示的模板 -->
        <div class="message-list" v-for="(message, index) in messageList" :key="index" v-if="!message.isPrivate || message.actUserId == userId">
      ...
      <div class="load-more">
        ...
        <el-button @click="loadMore" v-if="!isLoadMore && total != 0">没有更多了</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    this.listQuery.actUserId = this.userId
    this.getMessageList()
  },
  mounted() {
  },
  methods: {
    getMessageList() {
      const currentCounter = this.total
      getActivity(this.listQuery).then(res => {
        ...
        if (res.data.total === currentCounter) {
          this.loadMore = false
          return
        }
